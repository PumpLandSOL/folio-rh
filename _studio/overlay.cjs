...
(() => {
  ...
  s.textContent = ` + '`' + `
    #dmTitle{position:fixed;inset:0;z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#0a0a0a;opacity:0;transition:opacity .45s;text-align:center}
    #dmTitle.on{opacity:1}
    #dmTitle .t{font-family:'Newsreader',Georgia,serif;font-weight:300;font-size:74px;letter-spacing:-.02em;line-height:1.05;color:#f4f4f1;max-width:1000px}
    #dmTitle .t em{font-style:italic;color:#00a344}
    #dmTitle .s{font-family:'Hanken Grotesk',system-ui,sans-serif;font-weight:700;font-size:14px;letter-spacing:.24em;color:#00a344;margin-top:26px;text-transform:uppercase}
    #dmTitle .ln{width:120px;height:1px;background:#00c805;margin-top:30px}
    #dmCap{position:fixed;left:32px;bottom:32px;transform:translateY(24px);z-index:99998;max-width:640px;background:rgba(255,253,249,.96);border:1px solid #232323;border-left:3px solid #00c805;padding:16px 24px;opacity:0;transition:opacity .3s,transform .3s;box-shadow:0 18px 50px rgba(244,244,241,.14)}
    #dmCap.on{opacity:1;transform:translateY(0)}
    #dmCap .k{font-family:'Hanken Grotesk',system-ui,sans-serif;font-weight:700;font-size:10.5px;letter-spacing:.2em;color:#00a344;text-transform:uppercase}
    #dmCap .v{font-family:'Newsreader',Georgia,serif;font-weight:300;font-size:25px;color:#f4f4f1;margin-top:6px;line-height:1.3}
    #dmCap .v b{font-weight:400;color:#00a344;font-style:italic}
    ...
    #dmRing{position:fixed;z-index:99999;width:56px;height:56px;border:2px solid #00c805;border-radius:50%;pointer-events:none;opacity:0;transform:translate(-50%,-50%) scale(.3)}
  ...
  cur.innerHTML='<svg width="22" height="22" viewBox="0 0 22 22"><path d="M2 2 L2 17 L6 13 L9 20 L12 19 L9 12 L15 12 Z" fill="#f4f4f1" stroke="#0a0a0a" stroke-width="1.2"/></svg>';
