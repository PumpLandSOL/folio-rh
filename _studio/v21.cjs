...
.g{color:var(--gold2)}.r{color:#ff5000}
...
<div style="display:grid;grid-template-columns:1.15fr 1fr;gap:26px">
 <div class="card" style="border-color:var(--gold)"><span class="caps" style="font-size:11px">Dynamic buyback &amp; burn</span><h3>30% base → <span class="r">60%</span> when $FOLIO trades 20% below its 7-day average</h3>
  <svg viewBox="0 0 640 290" style="width:100%;height:auto;margin-top:8px;font-family:'JetBrains Mono'">
   <line x1="60" y1="250" x2="580" y2="250" stroke="#232323"/><line x1="60" y1="50" x2="60" y2="250" stroke="#232323"/>
   ${[30,40,50,60].map(s=>`<line x1="60" x2="580" y1="${Y(s)}" y2="${Y(s)}" stroke="#232323" stroke-dasharray="2 4"/><text x="50" y="${Y(s)+4}" font-size="11" fill="#7d7d78" text-anchor="end">${s}%</text>`).join('')}
   ${[-10,0,10,20,30].map(d=>`<text x="${X(d)}" y="270" font-size="11" fill="#7d7d78" text-anchor="middle">${d<=0?(d===0?'at avg':'+'+(-d)+'%'):'−'+d+'%'}</text>`).join('')}
   <line x1="${X(0)}" x2="${X(0)}" y1="50" y2="250" stroke="#00c805" stroke-dasharray="3 3"/><text x="${X(0)+6}" y="62" font-size="11" fill="#00a344">7-day average</text>
   <polyline points="${curve}" fill="none" stroke="#ff5000" stroke-width="3"/>
   <circle cx="${X(0)}" cy="${Y(30)}" r="5" fill="#00c805"/><circle cx="${X(20)}" cy="${Y(60)}" r="5" fill="#ff5000"/>
   <text x="${X(20)+10}" y="${Y(60)-12}" font-size="12" fill="#ff5000">60% · max burn</text>
